#!/usr/bin/env python3
"""Render gshare_predictor.vcd (captured from a real iverilog run) to
docs/gshare_predictor_waveform.png.

This is a GENUINE simulator capture: we parse the VCD that `make icarus`
produced and draw the directed window with matplotlib. Nothing is hand-modeled
-- every prediction, index, GHR value, and saturating-counter value shown is
read back out of the VCD the testbench dumped.

The window covers reset, the Phase-2 taken run of PC_A (watch the GHR fill
0000 -> 0001 -> 0011 -> 0111 -> 1111 and the folded index track it -- the
gshare fingerprint), and the first alternating branches of the Phase-3
correlation demo on PC_B.
"""
import os
import re
import matplotlib
...
VCD = "gshare_predictor.vcd"
OUT = os.path.join("docs", "gshare_predictor_waveform.png")

# Directed window in ns (random stress begins far past this).
T_END = 138
...
def main():
    ...
    fig, axes = plt.subplots(9, 1, figsize=(14, 10.5), sharex=True)
    fig.suptitle("Day24 - gshare correlating branch predictor  "
    ...
    bus_axis(axes[5], ghr,  "GHR",         "#e377c2", lambda v: f"{v:04b}")
    bus_axis(axes[6], pidx, "predict idx", "#9467bd", lambda v: f"{v}")
    bus_axis(axes[7], uidx, "update idx",  "#ff7f0e", lambda v: f"{v}")
    bus_axis(axes[8], uctr, "counter",     "#17becf",
    ...
    cap = ("Captured directed window. After reset every counter = WN and "
           "GHR = 0000. Phase 2 trains PC_A (pc=0x40, pc-index 0) taken: as "
           "each resolved outcome shifts into the GHR (0000 -> 0001 -> 0011 -> "
           "0111 -> 1111), the XOR-folded 'predict idx' walks 0->1->3->7->15 -- "
           "the gshare fingerprint (history moves the index). Each freshly "
           "visited counter starts at WN and is nudged WN->WT, and once the "
           "history saturates the same entry is re-tread and climbs toward ST, "
           "so predict_taken settles high. Phase 3 then begins the alternating "
           "T,N,T,N branch on PC_B (0x100): its GHR phase steers taken vs. "
           "not-taken occurrences into different counters -- the correlation a "
           "bimodal predictor cannot separate.")
