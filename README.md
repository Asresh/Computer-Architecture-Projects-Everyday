...
| 36 | Dual-Issue Instruction Fetch Queue | A two-wide elastic front-end queue carrying instruction and prediction metadata with simultaneous enqueue/dequeue and redirect flush recovery. | [day36-instruction_fetch_queue](day36-instruction_fetch_queue/) |
| 37 | Programmable Hardware Performance Monitor | A PMU with fixed cycle/retire counters, programmable event selectors, atomic snapshots, and sticky overflow interrupt reporting. | [day37-hardware_performance_monitor](day37-hardware_performance_monitor/) |
| 38 | Store-Set Memory-Dependence Predictor | An SSIT/LFST predictor that learns violating load/store pairs, merges dependence classes, and orders risky loads behind the youngest in-flight store. | [day38-store_set_predictor](day38-store_set_predictor/) |
| 39 | Fully-Associative Victim Cache | A compact side cache that converts L1 conflict misses into fast associative hits with atomic line swaps, dirty tracking, and round-robin replacement. | [day39-victim_cache](day39-victim_cache/) |
| 40 | Directory-Based Snoop Filter | A coherence directory that tracks per-line sharers and ownership, targets only necessary cache probes, and safely handles directory conflicts. | [day40-directory_snoop_filter](day40-directory_snoop_filter/) |
| 41 | Dual-Clock Asynchronous FIFO | A Gray-pointer CDC queue that safely transfers ordered data between unrelated clock domains with conservative full/empty backpressure. | [day41-asynchronous_fifo](day41-asynchronous_fifo/) |
| 42 | Burst DMA Engine | A parameterized memory-copy engine with ready/valid backpressure, variable-latency reads, buffered writes, and completion signaling. | [day42-burst_dma_engine](day42-burst_dma_engine/) |
| 43 | Credit-Based Virtual-Channel Scheduler | A credit-gated round-robin scheduler that safely shares a physical interconnect link across buffered virtual channels. | [day43-credit_based_vc_scheduler](day43-credit_based_vc_scheduler/) |
| 44 | Outstanding Coherence Transaction Tracker | An associative tagged table that tracks unordered snoop acknowledgements for multiple concurrent coherence operations, with completion backpressure and sticky protocol/timeout detection. | [day44-coherence_transaction_tracker](day44-coherence_transaction_tracker/) |
| 45 | Directory-Based Coherence Home Agent | A coherence controller that turns directory metadata into downgrade/invalidate probes, safe line replacement, memory refill, and tagged shared/unique responses. | [day45-coherence_home_agent](day45-coherence_home_agent/) |
