#!/usr/bin/env python3
"""Render docs/mem_coalescing_unit_waveform.png from the captured
mem_coalescing_unit.vcd.

Parses the VCD produced by the Icarus run, reconstructs each probed signal, and
draws a cycle-accurate timing diagram of the directed coalescing scenarios:
reset, then the seven directed warp requests (perfectly coalesced, fully
scattered, two-segment, masked-off lanes, broadcast, empty warp, strided-2).
For each request the diagram shows the input active_mask alongside the
registered response one cycle later (resp_valid, n_active, n_txn, leader_mask).
This is a REAL captured trace, not a hand-drawn diagram.
"""
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle

VCD = "mem_coalescing_unit.vcd"

# leaf name -> friendly label (first occurrence = tb scope)
WANT = {
    "clk": "clk",
    "req_valid": "req_valid",
    "active_mask": "active_mask",
    "resp_valid": "resp_valid",
    "n_active": "n_active",
    "n_txn": "n_txn",
    "leader_mask": "leader_mask",
}


def parse_vcd(path):
    with open(path) as f:
        text = f.read()
    header, _, body = text.partition("$enddefinitions")
    id_of, width = {}, {}
    for line in header.splitlines():
        s = line.strip()
        if s.startswith("$var"):
            p = s.split()
            w, vid, name = int(p[2]), p[3], p[4]
            if name in WANT and WANT[name] not in id_of:
                id_of[WANT[name]] = vid
                width[WANT[name]] = w
    id2lab = {v: k for k, v in id_of.items()}
    changes = {lab: [] for lab in id_of}
    t = 0
    for line in body.splitlines():
        s = line.strip()
        if not s:
            continue
        if s[0] == "#":
            t = int(s[1:])
        elif s[0] in "01xzXZ":
            if s[1:] in id2lab:
                changes[id2lab[s[1:]]].append((t, s[0]))
        elif s[0] in "bB":
            p = s.split()
            if len(p) == 2 and p[1] in id2lab:
                changes[id2lab[p[1]]].append((t, p[0][1:]))
    return changes, width


def val_at(seq, t):
    """last value at or before time t (string of bits or single bit)."""
    v = "x"
    for (tt, val) in seq:
        if tt <= t:
            v = val
        else:
            break
    return v


def to_int(bits):
    try:
        return int(bits, 2)
    except ValueError:
        return None


def main():
    changes, width = parse_vcd(VCD)

    # Sample on each clock rising edge (0 -> 1 transition of clk). Times in the
    # VCD are in ps; deriving edges from clk avoids hard-coding the timescale.
    clk_seq = changes.get("clk", [])
    edges = [t for (t, v) in clk_seq if v == "1"]
    # reset + the seven directed requests occupy the first ~20 cycles
    NCYC = 20
    edges = edges[:NCYC]

    labels = ["clk", "req_valid", "active_mask", "resp_valid",
              "n_active", "n_txn", "leader_mask"]
    is_bus = {"active_mask": True, "leader_mask": True,
              "n_active": True, "n_txn": True,
              "clk": False, "req_valid": False, "resp_valid": False}

    ncol = len(edges)
    row_h = 1.0
    fig_w = max(11, 1.05 * ncol)
    fig_h = 0.85 * len(labels) + 2.0
    fig, ax = plt.subplots(figsize=(fig_w, fig_h))

    C_HI = "#2e7d32"
    C_LO = "#9aa4ad"
    C_BUS = "#e8eef5"
    C_BUSE = "#3f6fb0"
    C_TXT = "#10233b"

    for r, lab in enumerate(labels):
        y = (len(labels) - 1 - r) * (row_h + 0.35)
        ax.text(-0.4, y + row_h / 2, lab, ha="right", va="center",
                fontsize=11, family="monospace", color=C_TXT)
        seq = changes.get(lab, [])
        for c, t in enumerate(edges):
            x = c
            if lab == "clk":
                # draw a rising-edge clock: low first half, high second half
                ax.plot([x, x + 0.5, x + 0.5, x + 1.0],
                        [y, y, y + row_h, y + row_h], color="#456", lw=1.6)
                continue
            raw = val_at(seq, t)
            if not is_bus[lab]:
                hi = (raw == "1")
                col = C_HI if hi else C_LO
                yy = y + (row_h if hi else 0.0)
                # draw level line
                ax.add_patch(Rectangle((x, y), 1.0, row_h,
                                       facecolor="none", edgecolor="none"))
                ax.plot([x, x + 1.0], [yy, yy], color=col, lw=2.4)
                ax.plot([x, x], [y, y + row_h], color=col, lw=0.8, alpha=0.5)
            else:
                iv = to_int(raw)
                if lab in ("active_mask", "leader_mask"):
                    w = width.get(lab, 8)
                    txt = format(iv, "0{}b".format(w)) if iv is not None else "x"
                else:
                    txt = str(iv) if iv is not None else "x"
                nonzero = (iv not in (None, 0))
                fc = C_BUS
                ec = C_BUSE if nonzero else "#c2cbd4"
                ax.add_patch(Rectangle((x + 0.03, y + 0.12), 0.94, row_h - 0.24,
                                       facecolor=fc, edgecolor=ec, lw=1.2))
                ax.text(x + 0.5, y + row_h / 2, txt, ha="center", va="center",
                        fontsize=8.5, family="monospace",
                        color=C_TXT if nonzero else "#7a848d")

    # cycle axis
    ytop = len(labels) * (row_h + 0.35)
    for c in range(ncol):
        ax.axvline(c, color="#dde3ea", lw=0.6, zorder=0)
    ax.axvline(ncol, color="#dde3ea", lw=0.6, zorder=0)
    for c in range(ncol):
        ax.text(c + 0.5, ytop + 0.05, "c{}".format(c), ha="center", va="bottom",
                fontsize=8, color="#5a6673")

    # annotate the directed scenarios. Sampling is post-edge, so each request's
    # active_mask and its registered response land in the same column.
    notes = [
        (2, "reset"),
        (5, "req1: coalesced\n8 lanes -> 1 txn"),
        (7, "req2: scattered\n-> 8 txns"),
        (9, "req3: 2 lines\n-> 2 txns"),
        (11, "req4: even mask\n4 lanes -> 1 txn"),
        (13, "req5: bcast\n-> 1 txn"),
        (15, "req6: empty\n-> 0 txns"),
        (17, "req7: stride2\n-> 2 txns"),
    ]
    for cyc, txt in notes:
        if cyc < ncol:
            ax.text(cyc + 0.5, -1.35, txt, ha="center", va="top", fontsize=7.6,
                    color="#3a4a5e",
                    bbox=dict(boxstyle="round,pad=0.25", fc="#f2f6fb",
                              ec="#c2cbd4", lw=0.7))

    ax.set_xlim(-3.0, ncol + 0.2)
    ax.set_ylim(-2.6, ytop + 0.7)
    ax.axis("off")
    ax.set_title(
        "Day15 - GPU Memory Coalescing Unit: captured simulation timing "
        "(Icarus Verilog VCD)\n"
        "warp request (active_mask) and its registered coalescing response "
        "(n_active, n_txn, leader_mask), sampled at each clock edge",
        fontsize=11, color=C_TXT, pad=14)

    fig.tight_layout()
    fig.savefig("docs/mem_coalescing_unit_waveform.png", dpi=140,
                bbox_inches="tight", facecolor="white")
    print("wrote docs/mem_coalescing_unit_waveform.png")


if __name__ == "__main__":
    main()
